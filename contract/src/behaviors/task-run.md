...
## Ceremony scaling guidance / 仪式缩放指导 (v6.11.4)

Engine contract mandates a **minimum task card format**: `WRITE-SET`, optional `FORBIDDEN`, `AC + verify` commands, and decision/plan/domain references. `GOAL` is used by hooks for session anchoring (240-char truncation); `CONSTRAINTS` is documentary. Everything beyond these fields is a **project-chosen scaling enhancement**, not an engine requirement.

Common scaling enhancements that projects may adopt based on risk:

| Enhancement | When useful | Engine stance |
|-------------|-------------|---------------|
| Parameter freeze list (base tip, digest, fixture hash) | Large changes (>500 lines), schema migrations, new product surfaces | Not checked, not required |
| Multi-role review (isolated verifier + acceptance owner) | High-risk tasks where "author self-verify" is insufficient | `engine verify` only runs AC commands; role count is project choice |
| Lightweight follow-up tracking (FU-level items) | Audits producing many small tracking items that don't warrant full T-cards | Use `depends-on` chains or CONTEXT "pending" section; no engine-level FU format |
| Risk-graded ceremony weight | Projects with mixed high/low risk tasks in parallel | Engine treats all active cards equally; grading is project governance |

**Agent behavior rule**: When executing a task card, the agent MUST treat only the contract-minimum fields as mandatory. If a project's task card contains additional fields (freeze parameters, review_mode, FU references), the agent SHOULD respect them as project conventions — but MUST NOT refuse to proceed when they are absent on a low-risk task. When in doubt, ask the architect whether a field is project-mandatory or optional.

**Anti-pattern**: Inventing engine-sounding justifications for project ceremony ("the engine requires a 12-parameter freeze before done"). The engine requires: every AC has PASS evidence or an approved exemption. Nothing more.
