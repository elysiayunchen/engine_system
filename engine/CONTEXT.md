...
- 待验证：D-029(approved 2026-07-20)多 Claude Code 实例并行抢写引擎记忆三件套(CONTEXT/HANDOFF/ENGINE_MAP)的隔离机制。主线方案 A(多会话锁 + worker 自动降级 + PreToolUse 双信号)approved。**T-036 v6.11.0 paused**(18 个 AC):AC-1 ✅ 契约源 30-operational.md 加 MULTI-SESSION ISOLATION 段(atomic lock + 协调者/worker 角色 + pid+StartTime 双信号 + PreToolUse 双信号 + fail-open/closed + D-028 三文件 worker 写入边界 + kill switch);AC-2 部分实现(SessionStart hook 已加 lock 检测代码但未 commit);review 发现 P1-P7 问题待修(P1 AC-2 verify 4 份范围错位 + P2 worker_key 算法与 Stop hook safe_id 不一致 + P3 atomic 创建+写入两步分离竞态 + P4 AC-1 verify 缺 3 grep + P5 AC-2 verify 缺 atomic 关键字 + P6 AC-2 verify 缺 tombstone grep + P7 AC-18 缺 PowerShell 测试)。expiry 2026-11-30,早期预警 2026-09-15。
