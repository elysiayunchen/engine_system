...
| 上次完成 | **v6.11.5 patch(T-043 issue #10 P038 parse_task_patterns 支持 YAML frontmatter 多行 write-set)**:T-043 6/6 AC PASS。修复 GitHub issue #10 的 P038 — pre-commit `parse_task_patterns`(L35-77)两个解析分支(inline grep L37-41 + awk markdown L42-55)都不支持 YAML frontmatter 多行 `write-set:` 格式,导致用该格式的任务卡被误报「no readable WRITE-SET」拦截非相关 commit。awk 分支重写:新增 `in_frontmatter_block` 边界状态(`^---$` 开闭切换)+ `in_frontmatter_field` 字段头匹配(`^field:$` 缩进 `- ` 列表项收集)+ `field_lc=tolower(field)` 大小写不敏感比较;原 markdown `## field` section 分支保留并改 case-insensitive。补充 reporter 补丁缺失的两个细节:case 敏感性(大写 WRITE-SET 调用需匹配小写 write-set: frontmatter)+ frontmatter 边界检测(`^---$` 限定避免 body 误匹配)。不改 P037 legacy fallback(L89-94,留 T-044/D-032 已批准)。check.sh 0 failures 2 warnings 非阻塞 PASS。 |
| 进行中 | 无 active 任务卡。下一步可选方向:`git commit` + 可选 `git tag v6.11.5 && git push && git push --tags` 触发 GitHub Release workflow;回复 GitHub issue #10(P038 修复在 v6.11.5,P037 待 T-044);开 T-044(实施 D-032 approved — 移除 legacy fallback L89-94 + fail-open);或 push T-042 v6.11.4 + 回复 issue #9(尚未 push)。 |
| 阻塞 | 无。 |
