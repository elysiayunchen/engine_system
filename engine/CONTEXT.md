...
| 上次完成 | **v6.12.2(T-050 tombstone 生命周期双重 bug 修复)**:共同根因——把「历史 transition 记录」当成「active 状态信号」治理。Bug A:SessionStart hook 获取 fresh/same-sid coordinator 锁时不清理旧 tombstone,只有 `engine assume-coordinator` 命令清理 → 安静 24h+ 仓库 doctor 必 FAIL(本仓 36h 后触发实证);修复:fresh + same-sid resume 两条路径加 `rm -f .cache/session.tombstone`(对称 Stop hook 写入,stale-recovery 路径不动)。Bug B:Doctor `check_multi_session_isolation` 把 >24h tombstone 报 "previous coordinator exited abnormally" 并 FAIL,但 `coordinator-exited` 是 Stop hook 写的**正常退出**标记;且契约 #17 原文说 WARN 代码却 FAIL(契约/代码不一致);修复:`tombstone_is_fail` 变量(cv ∈ [6.11.0, 6.12.2) 保持旧 FAIL,cv ≥ 6.12.2 WARN)+ 消息删 "exited abnormally" 改 "historical transition record"。契约 #17 重写(tombstone 是 historical transition records, NOT active-state signals; SessionStart MUST clean);contract-version 升 6.12.2;migrator sh/ps1 + plugin 镜像同步;ENGINE_FILE_SYSTEM_v5 + AGENT_ADAPTERS 文档同步。verify 9/9 PASS。前序 **v6.12.1(T-049 issue #11 门禁静默失效家族九项修复)**:verify 全 SKIP 改 exit 3 显式 parse-failure + AC id 字母分组;hook/doctor 统一三格式 WRITE-SET 解析;status 检测全站点行首锚定 + 同卡 active+done 冲突 FAIL;migrator contract-version 改 engine/VERSION 优先;AC 模板反套套逻辑三问。前序 **v6.12.0(T-048 多任务卡并行 union gating + 会话租约液性修复,D-035)**:根治「激活一张任务卡后其他 agent 被全面拦截」。 |
| 进行中 | T-050 收尾提交。下一步:push main 验证 CI → 可选 tag v6.12.2;真实双会话并行试用(两实例各持一张卡)采集 TTL/overlap WARN 手感;或规划 Q2 真实大库试点 / D-018 批量处理。 |
| 阻塞 | 无。 |
