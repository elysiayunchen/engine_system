...
17. Multi-session isolation (v6.11.0+ D-029/T-036, revised v6.12.0 D-035/T-048, v6.12.2 T-050): `check_multi_session_isolation` enforces `engine/.cache/sessions/` exists with coordinator lock `engine/.cache/session.lock` (5-field format `pid|session_id|role|started_at|task_id`).
    - **Pid field**: diagnostic only — liveness judged by lease freshness (lock mtime / holder `.hb` heartbeat mtime within ENGINE_SESSION_TTL_MIN, default 120min), NEVER by pid liveness (recorded pid is transient hook shell).
    - **Tombstone files** (`engine/.cache/session.tombstone`): historical transition records (`coordinator-exited`/`stale-recovered`/`forced-replaced`), NOT active-state signals — lock file + lease mtime is source of truth. Tombstone >24h triggers WARN (auto-cleans on next coordinator start).
    - **SessionStart hook**: MUST use atomic exclusive creation (`set -C`/`noclobber` bash; `FileMode.CreateNew`+`FileShare.None` PowerShell) for coordinator/worker roles; MUST remove own `.role=worker` flag on every coordinator acquisition (acquire/same-sid re-entry/stale takeover); MUST clean existing tombstone on fresh acquisition (acquire/same-sid paths; stale-takeover overwrites with `stale-recovered`); MUST GC session files >7 days.
    - **PreToolUse hook**: MUST treat session as worker when EITHER signal present (OR, not AND): `agent_id` non-empty OR `.cache/sessions/<key>.role=worker` marker exists; MUST renew heartbeat on every tool call; MUST validate lease at write time for shared singletons (holder writes; non-holder blocked while fresh; free/stale claimed atomically).
    - **Task gating**: union across ALL active cards (one card per parallel session). Kill switch `ENGINE_DISABLE_MULTI_SESSION=1` or `engine/.cache/multi-session.disabled` flag → SessionStart skips lock detection, all sessions degrade to single-session (fail-open).
    - **Migration grace period**: tombstone staleness — `cv < 6.11.0` WARN (advisory), `6.11.0 <= cv < 6.12.2` FAIL, `>= 6.12.2` WARN (historical record); other multi-session checks — `cv < 6.12.0` fail-open, `>= 6.12.0` FAIL (D-028 §9).
