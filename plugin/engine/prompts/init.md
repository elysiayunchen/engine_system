<!-- engine/prompts/init.md: compiled from contract/src/ (agent-preamble.md + [0-9]*.md) by engine compile. Do not edit dist directly; edit src and recompile. -->
...
**Quality Gate Enforcement (v6.24, extends S4):** Beyond per-AC verify, a done-transition requires an aggregated gate registry (`engine/evidence/T-NNN/GATE.json`, written by `engine gate T-NNN`) that rolls up all applicable quality gates (verify, review, review-agent, prove) into a single pass/block verdict with provenance (writer=engine-gate, commit=HEAD). Pre-commit blocks the done-transition when GATE.json is missing, status≠pass, or provenance is stale. Gates whose tooling is unavailable are skipped (not blocked); docs-only WRITE-SETs skip code gates. The seal config (`engine/gate/config.json`) governs --no-verify bypass detection: default sealed (bypass → subsequent commits blocked until resolved); `seal.override_authored: true` opts out per project.
