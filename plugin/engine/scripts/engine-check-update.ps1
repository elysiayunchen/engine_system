...
# never yields a false update hint; non-numeric versions are returned as-is and
# compared conservatively as an unknown shape (D-015).
...
function Compare-Normalized-Version([string]$left, [string]$right) {
  $leftParts = @($left.Split('.'))
  $rightParts = @($right.Split('.'))
  $maxParts = [Math]::Max($leftParts.Count, $rightParts.Count)
  for ($i = 0; $i -lt $maxParts; $i++) {
    $leftPart = if ($i -lt $leftParts.Count) { [int64]$leftParts[$i] } else { 0 }
    $rightPart = if ($i -lt $rightParts.Count) { [int64]$rightParts[$i] } else { 0 }
...
$localNormalized = Normalize-Version $localVersion
$remoteNormalized = Normalize-Version $remoteVersion
...
if (($localNormalized -match '^[0-9]+(\.[0-9]+)*$') -and ($remoteNormalized -match '^[0-9]+(\.[0-9]+)*$')) {
  $comparison = Compare-Normalized-Version $localNormalized $remoteNormalized
  if ($comparison -lt 0) {
  ...
  if ($comparison -gt 0) {
    Write-Output "Remote version $remoteVersion is older than local $localVersion; no downgrade recommended."
    exit 0
  }
}

Write-Error "Version comparison unavailable for local '$localVersion' and remote '$remoteVersion'."
exit 7
