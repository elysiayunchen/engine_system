...
17. Multi-session isolation (v6.11.0+ D-029/T-036, revised v6.12.0 D-035/T-048): `check_multi_session_isolation` enforces that `engine/.cache/sessions/` directory exists with the coordinator lock file `engine/.cache/session.lock` following the 5-field format (`pid|session_id|role|started_at|task_id`). The pid field is diagnostic only — lock liveness MUST be judged by lease freshness (newest of lock mtime / holder `.hb` heartbeat mtime within ENGINE_SESSION_TTL_MIN, default 120 minutes), NEVER by pid liveness (the recorded pid is the transient hook shell). Tombstone files (`engine/.cache/session.tombstone`) MUST be inspected for staleness — any tombstone older than 24 hours is treated as a stale crash marker and triggers WARN (clear via `engine assume-coordinator`). SessionStart hook MUST use atomic exclusive creation (`set -C` / `noclobber` on bash; `FileMode.CreateNew` + `FileShare.None` on PowerShell) to assign coordinator/worker roles, MUST remove the session's own `.role=worker` flag on every coordinator acquisition path (acquire / same-sid re-entry / stale takeover), and MUST GC session files older than 7 days. PreToolUse hook MUST treat a session as worker when EITHER signal is present (OR, not AND): `agent_id` non-empty OR `.cache/sessions/<key>.role=worker` marker file exists, MUST renew the session heartbeat on every tool call, and MUST validate the lease at write time for shared singletons (holder writes; non-holder blocked while fresh; free/stale claimed atomically). Task gating is union across ALL active cards (one card per parallel session). Kill switch `ENGINE_DISABLE_MULTI_SESSION=1` or `engine/.cache/multi-session.disabled` flag file causes SessionStart hook to skip lock detection and all sessions degrade to single-session mode (fail-open equivalent). Migration grace period: `contract-version < 6.12.0` keeps prior behavior (fail-open), `>= 6.12.0` FAIL (D-028 §9).
