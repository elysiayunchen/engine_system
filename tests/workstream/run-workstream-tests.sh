...
[ -f "$r/engine/workstreams/T-001/worker-a/CONTEXT.md" ] && [ -f "$r/engine/workstreams/T-001/worker-a/HANDOFF.md" ] && ok "sh files" || bad "sh files"
grep -q 'isolate parallel worker memory' "$r/engine/workstreams/T-001/worker-a/CONTEXT.md" && ok "sh goal copied" || bad "sh goal copied"
printf '\ncustom marker\n' >> "$r/engine/workstreams/T-001/worker-a/CONTEXT.md"
(cd "$r" && bash engine/bin/engine workstream T-001 worker-a >/dev/null)
grep -q 'custom marker' "$r/engine/workstreams/T-001/worker-a/CONTEXT.md" && ok "sh idempotent preserve" || bad "sh idempotent preserve"
