...
contains "$verify" 'ENGINE_VERIFY_ACTIVE_TASK'
contains "$verify" 'ENGINE_VERIFY_RECURSE_STACK'
contains "$verify" 'engine/review/evidence/*'
contains "$verify" 'invalidate stale later-AC snapshots'
contains "$close" 'refresh_evidence_manifest'
contains "$close" 'The stage'"'"'s exit code must be independent of display I/O.'
contains "$doctor" 'GATE registry deferred during active verification'
contains "$REPO_ROOT/engine/scripts/engine-close.ps1" 'Refresh-EvidenceManifest'
contains "$REPO_ROOT/engine/scripts/engine-close.ps1" 'ConvertTo-Json -Depth 8 -Compress'
contains "$drift_sh" 'historical_snapshot=0'
contains "$drift_sh" 'legacy evidence provenance.commit mismatch'
contains "$drift_sh" 'WRITE-SET globs are metadata coverage'
